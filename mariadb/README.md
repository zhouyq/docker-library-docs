...
```console
$ docker run --detach --name some-mariadb --env MARIADB_USER=example-user --env MARIADB_PASSWORD=my_cool_secret --env MARIADB_ROOT_PASSWORD=my-secret-pw  mips64le/mariadb:latest
```
...
```console
$ docker run -it --rm mips64le/mariadb mysql -h <server container IP> -u example-user -p
```
...
```console
$ docker run --name some-mariadb -v /my/custom:/etc/mysql/conf.d -e MARIADB_ROOT_PASSWORD=my-secret-pw -d mips64le/mariadb:latest
```
...
```console
$ docker run --name some-mysql -e MARIADB_ROOT_PASSWORD_FILE=/run/secrets/mysql-root -d mips64le/mariadb:latest
```
...
2.	Start your `mips64le/mariadb` container like this:
	...
	```console
	$ docker run --name some-mariadb -v /my/own/datadir:/var/lib/mysql -e MARIADB_ROOT_PASSWORD=my-secret-pw -d mips64le/mariadb:latest
	```
...
$ docker run --name some-mariadb -v /my/own/datadir:/var/lib/mysql -v some-mariadb-socket:/var/run/mysqld -e MARIADB_MYSQL_LOCALHOST_USER=1 -e MARIADB_MYSQL_LOCALHOST_GRANTS="RELOAD, PROCESS, LOCK TABLES, BINLOG MONITOR" -e MARIADB_ROOT_PASSWORD=my-secret-pw -d mips64le/mariadb:latest
```
...
$ docker run --rm some-mariadb-backup -v some-mariadb-backup:/backup mips64le/mariadb:latest chown mysql: /backup
```

To perform the backup:

```console
$ docker run --user mysql -v some-mariadb-socket:/var/run/mysqld -v some-mariadb-backup:/backup -v /my/own/datadir:/var/lib/mysql --rm mips64le/mariadb:latest mariabackup --backup --target-dir=/backup
```
...
```console
$ docker run --user mysql --rm -v some-mariadb-backup:/backup mips64le/mariadb:latest mariabackup --prepare --target-dir=/backup
```

Now that the image is prepared, start the container with both the data and the backup volumes and restore the backup:

```console
$ docker run --user mysql --rm -v /my/new/datadir:/var/lib/mysql -v some-mariadb-backup:/backup mips64le/mariadb:latest mariabackup --copy-back --target-dir=/backup
```

With `/my/new/datadir` containing the restored backup, start normally as this is an initialized data directory:

```console
$ docker run --name some-mariadb -v /my/new/datadir:/var/lib/mysql -d mips64le/mariadb:latest
```
...
```console
$ docker run --rm -v /my/own/datadir:/var/lib/mysql -v /my/own/passwordreset.sql:/passwordreset.sql:z mips64le/mariadb:latest --init-file=/passwordreset.sql
```
...
MariaDB has many plugins, most are not enabled by default, some are in the mips64le/mariadb container, others need to be installed from additional packages.
...
To see which plugins are available in the mips64le/mariadb:

```console
$ docker run --rm mips64le/mariadb:latest ls -C /usr/lib/mysql/plugin
```
...
```console
$ docker run --name some-mariadb -e MARIADB_ROOT_PASSWORD=my-secret-pw --network=host -d mips64le/mariadb:latest --plugin-load-add=simple_password_check
```
...
```console
$ docker run --rm mips64le/mariadb:latest sh -c 'apt-get update -qq && apt-cache search mariadb-plugin'
```

### Creating a image with plugins from additional packages

A new image needs to be created when using additional packages. The mips64le/mariadb image can be used as a base however:

In the following the [CONNECT Storage Engine](https://mariadb.com/kb/en/connect/) is installed:

```dockerfile
FROM mips64le/mariadb:latest
RUN apt-get update && \
